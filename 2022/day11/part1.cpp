...
void
monkey_inspection(std::vector<Monkey> &monkeys, Monkey &monkey)
{
        while (!monkey.items.empty()) {
                ...
                item = operation(item, monkey.op_type, monkey.op_num) / 3;
...
void
monkeys_inspection(std::vector<Monkey> &monkeys)
{
        for (auto &monkey : monkeys)
                monkey_inspection(monkeys, monkey);
}
