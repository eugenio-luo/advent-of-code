...
bool calcTrajectory(Pos v, const Pos& t1, const Pos& t2, ll& maxY) {
    Pos init = Pos(0, 0);
    ll y = 0;
    ...
    while (init.x <= t2.x && init.y >= t1.y) {
        ...
        if (y < init.y) y = init.y;
        
        if (v.x != 0) v.x += (v.x > 0) ? -1 : 1;
        v.y -= 1;
...
ll highestY(const Pos& t1, const Pos& t2) {
    ll maxY = 0;
    
    for (ll x{}; x < maxTries; ++x) {
        for (ll y{-maxTries}; y < maxTries; ++y) {
            calcTrajectory(Pos(x, y), t1, t2, maxY);
        }
    }
...
ll solve(std::istream& input) {
    ...
    return highestY(initTarget, endTarget);
}
