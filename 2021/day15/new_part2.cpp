...
typedef std::vector<Vertex> VerVec;

typedef std::priority_queue<Vertex, VerVec, std::greater<Vertex>> PrioQueue;

void nextVertex(const IntVec& cavern, Vertex& v, IntVec& prev,
                int next) {
...
void vertex(const IntVec& cavern, Vertex& v, IntVec& prev,
            BoolVec& visited, int size) {

    visited[v.second] = true;
    
    if (v.second % size != size - 1) {
        nextVertex(cavern, v, prev, v.second + 1);
    }
...
long long path(const IntVec& cavern, int size, int bandwidth) {
    ...
    shortestPath.push({0, 0});

    while (!shortestPath.empty()) {
        Vertex v = shortestPath.top();
        shortestPath.pop();

        vertex(cavern, v, prev, visited, size);
    }
